...
#2. Find the largest number that when squared it is smaller or equal to the first number or pair. This will give you the first number in your answer. The first number on the left is 3. The highest square that goes into it is one because 1 x 1 = 1, while 2 x 2 = 4.
	...
	#7. Find the number that fills the blanks. This number will be the next digit in the solution for √361. The number that will fill the blanks will be the same, and it will be the highest digit where the factors are less than or equal to the number in step four. In this example, the goal number is 261. We will start with 9, so the equation will look like 29 x 9 = 261.
	#8. Put the number next to the first digit. In this example, the square is 19.
a = 361
SquareRootByLongDivision(a)		
